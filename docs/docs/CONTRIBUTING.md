...
Welcome! This guide provides all the details you need to contribute effectively to the project. Thank you for helping us make **codeuiapp** a better tool for developers worldwide. 💡
...
Interested in maintaining and growing the project? Fill out our [Contributor Application Form](https://support.wromo.com).
...
```bash
docker run -p 5173:5173 --env-file .env.local codeuiapp-ai:development
```
...
**Option 2: Direct Docker Build Command**  
```bash
docker build . --target codeuiapp-ai-production
```
...
```bash
docker run -p 5173:5173 --env-file .env.local codeuiapp-ai:production
```
