...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifyCodeuiappActions } from './utils';
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      if (contextOptimization) {
        content = simplifyCodeuiappActions(content);
      }
