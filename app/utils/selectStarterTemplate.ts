...
export async function getTemplates(templateName: string, title?: string) {
  ...
  // exclude    .codeuiapp
  filteredFiles = filteredFiles.filter((x) => x.path.startsWith('.codeuiapp') == false);

  // check for ignore file in .codeuiapp folder
  const templateIgnoreFile = files.find((x) => x.path.startsWith('.codeuiapp') && x.name == 'ignore');
...
<codeuiappArtifact id="imported-files" title="${title || 'Importing Starter Files'}" type="bundled">
${filesToImport.files
  .map(
    (file) =>
      `<codeuiappAction type="file" filePath="${file.path}">
${file.content}
</codeuiappAction>`,
  )
  .join('\n')}
...
`;
  let userMessage = ``;
  const templatePromptFile = files.filter((x) => x.path.startsWith('.codeuiapp')).find((x) => x.name == 'prompt');
